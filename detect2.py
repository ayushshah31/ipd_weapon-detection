            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_crop:
                        ...
                        #                     [-1, 8, -1],
                        #                     [-1, -1, -1]])
                        # filtered_image = cv2.medianBlur(imc, 5)
                        # sharpened_image = cv2.filter2D(filtered_image, -1, kernel)
                        
                        # Adjust the brightness and contrast of the image
                        # alpha = 1  # Contrast control (1.0-3.0)
                        # beta = 10  # Brightness control (0-100)
                        # bright_contrast = cv2.convertScaleAbs(imc, alpha=alpha, beta=beta)
                        # edge_image = cv2.filter2D(imc, -1, kernel2)
                        ...
                        # Bilateral filter
                        # sharpened_image = cv2.filter2D(imc, -1, kernel)
                        # filtered = cv2.bilateralFilter(sharpened_image, 9, 75, 75)
                        ...
                        savePath = increment_path(save_dir / 'crops' / names[c] / f'{p.stem}.jpg')
